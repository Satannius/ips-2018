...
let rec compileExp  (e      : TypedExp)
  ...
  | Constant (BoolVal b, pos) ->
      ...
      if b then [Mips.LI (place, "1")] // if b = true
  ...
  *)
  | Times (e1, e2, pos) ->
      let t1 = newName "multiply_L"
      let t2 = newName "multiply_R"
      let code1 = compileExp e1 vtable t1
      let code2 = compileExp e2 vtable t2
  ...
  *)
  ...
  | Replicate (n_exp, a_exp, tp, pos) ->
    ...
    *)
    ...
    (* init counter i and array address in register *)
    let addr_reg = newName "addr_reg"
    let i_reg = newName "i_reg"
    ...
    (* Enter loop. Check if i_reg < size_reg. If true, continue
    ...
    let loop_footer = [ Mips.ADDI(addr_reg, addr_reg,
  ...
  *)
  | Filter (farg, arr_exp, elem_type, pos) ->
      ...
      let arr_reg  = newName "arr_reg"  (* address of array *)
      let elem_reg = newName "elem_reg" (* address of single element *)
      let res_reg  = newName "res_reg"  (* resulting array *)
      let tmp_reg = newName "tmp_reg"
      let truefalse_reg = newName "truefalse_reg" (* result of f(a) *)
      let arr_code = compileExp arr_exp vtable arr_reg

      let get_size = [ Mips.LW (size_reg, arr_reg, "0") ]

      let addr_reg = newName "addr_reg" (* address of element in new array *)
      let j_reg = newName "j_reg" (*Amount of true's returned by the function*)
      let i_reg = newName "i_reg"
      ...
      let loop_header = [ Mips.LABEL (loop_beg)
                        ; Mips.SUB (tmp_reg, i_reg, size_reg) //tmp_reg = i_reg - size_reg
                        ; Mips.BGEZ (tmp_reg, loop_end) //if tmp_reg >= 0 goto loop_end
                        ]

      (* filter is 'arr[i] = (f(old_arr[i]) => true)' *)
      let loop_filter0 =
          match getElemSize elem_type with
              | One -> Mips.LB (res_reg, elem_reg, "0")
                            :: applyFunArg(farg, [res_reg], vtable, truefalse_reg, pos)
                            @ [ Mips.ADDI (elem_reg, elem_reg, "1") ]
              | Four -> Mips.LW (res_reg, elem_reg, "0")
                            :: applyFunArg(farg, [res_reg], vtable, truefalse_reg, pos)
                            @ [ Mips.ADDI (elem_reg, elem_reg, "4") ]
      ...
      let loop_filter1 =
          match getElemSize elem_type with
              | One  -> [ Mips.BEQ (truefalse_reg, "0", loop_footer) //if (f(a)){GOTO loop_footer}
              ...
              | Four -> [ Mips.BEQ (truefalse_reg, "0", loop_footer) //if (f(a)){GOTO loop_footer}
